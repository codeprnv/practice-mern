...
import { fetchTrending } from "../../utils/Data/helper.js";
...
import { truncateText, extractYear } from "../../utils/Data/helper.js";
          ...
          >
            {data?.results?.slice(0, 7).map((item) => (
              <SwiperSlide key={item?.id}>
                <div className="pointer-events-none absolute top-0 left-0 z-0 h-full w-full bg-gradient-to-r from-black/20 to-transparent transition-colors duration-200 ease-in" />
                <div className="absolute bottom-20 mx-2 my-2 flex h-1/5 w-full flex-col gap-1 px-3 lg:bottom-16">
                  <div className="z-10 flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-white md:text-xl">
                      {item?.omdb?.imdbRating && item.omdb.imdbRating !== "N/A"
                  ...
                  <div className="mt-3 flex w-full items-center gap-3">
                    ...
                    {item?.omdb?.Genre.split(", ").map((genre, index) => (
                      <Link
                        key={index}
